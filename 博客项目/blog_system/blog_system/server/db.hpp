	...
	public:
		...
		bool SelectAll(Json::Value* blogs,
			...
			if (tag_id==""){
                ...
                sprintf(sql,"select blot_id,title,tag_id,create_time from blog_table where tag_id=%d",std::stoi(tag_id));
			}
		...
		bool Selectone(int32_t blog_id,Json::Value* blogs){
			...
			MYSQL_ROW row=mysql_fetch_row(result);
			(*blogs)["blog_id"]=atoi(row[0]);
			(*blogs)["title"]=row[1];
			(*blogs)["content"]=row[2];
			(*blogs)["title"]=atoi(row[3]);
			(*blogs)["create_time"]=row[4];
			return true;
		}
