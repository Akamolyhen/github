...
int main()
{
	Menu();
	int SendLen;
	struct sockaddr_in severaddr,recvaddr;
	pthread_t Psend;
	if((sockfd=socket(AF_INET,SOCK_STREAM,0))==-1)
	{
		pirntf("Falied to creat server socket\n");
		exit(0);
	}
	...
	if(connect(sockfd,(struct sockaddr *)&serveraddr,sizeof(struct sockaddr))==-1)
{
	...
	while (1) {
		...
		{
			fprintf(stdout, "[User ID:%d][Say:]%s", MyRecvMessage->_Sockfd, RecvBuffer + sizeof(struct Message));
		}
